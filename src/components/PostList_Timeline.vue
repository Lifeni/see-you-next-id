<template>
  <div id="PostList">
    ...
    <div class="post" v-for="(post, index) in posts" :key="post.id">
      ...
      <button class="show-reply" @click="showReply(index)">{{showReplyFlag[index]?'收起':'查看'}}回应</button>
      <button class="get-emoji" @click="showAddEmoji(index)">+ Emoji</button>
      <div class="emoji" v-for="emoji in info.emoji" :key="emoji.id" v-show="post.emoji[emoji.id].count">
        {{emoji.code}} ×
...
export default {
  ...
  methods: {
    ...
    showAddEmoji(index) {
      this.$set(this.showAddEmojiFlag, index, !this.showAddEmojiFlag[index]);
    },
    addEmoji(postId, emojiId) {
      ...
      this.$http({
        url: "https://api.lifeni.top/emoij",
        method: "post",
        data: {
          user: this.userId,
